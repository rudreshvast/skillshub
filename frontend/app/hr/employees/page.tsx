...
function EmployeeDirectoryContent() {
  ...
  const { data: peopleData, isLoading, error } = useQuery<PeopleListResponse>({
    queryKey: ['employees-directory', activeFilters, page],
    queryFn: async () => {
      const response = await api.get(`/employees/directory?${buildQueryParams()}`, {
  ...
  const incompleteCount = peopleData?.employees.filter(e => !e.profile_complete).length ?? 0
    ...
    <div className="space-y-6 py-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage and view team members</p>
        </div>
        {peopleData && (
      ...
      {incompleteCount > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="text-sm text-amber-900">
            ⚠️ {incompleteCount} employee{incompleteCount !== 1 ? 's' : ''} with incomplete profile{incompleteCount !== 1 ? 's' : ''}
          </p>
        </div>
      )}
