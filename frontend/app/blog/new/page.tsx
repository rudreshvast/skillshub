...
export default function NewArticlePage() {
  ...
  const saveDraft = useCallback(async (data: object, id: number | null) => {
    setSaveStatus('saving');
    try {
      if (!id) {
        const res = await api.post('/blog', data, { headers });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      saveDraft({ title, content, tags, department_tags: deptTags }, articleId);
    }, 2000);
    return () => clearTimeout(timer);
  }, [title, content, tags, deptTags, articleId, saveDraft]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let id = articleId;
      if (!id) {
        const res = await api.post('/blog', { title, content, tags, department_tags: deptTags }, { headers });
        id = res.data.id;
      } else {
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-[#EBEEF0]">
            <button
              type="button"
              onClick={() => saveDraft({ title, content, tags, department_tags: deptTags }, articleId)}
